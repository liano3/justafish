...
function renderIcon(iconSpriteUrl, iconId, className = '') {
    const classAttribute = className ? ` class="${className}"` : '';
    return `<svg${classAttribute} aria-hidden="true" focusable="false"><use href="${escapeHtml(iconSpriteUrl)}#${iconId}"></use></svg>`;
}
...
function renderProfileLinks(links, iconSpriteUrl) {
    return links.filter(link => link.url).map((link, index) => {
        ...
        const icon = iconId ? renderIcon(iconSpriteUrl, iconId) : '';
...
function renderResumeContacts(profile, text, iconSpriteUrl) {
    const contacts = [];
    const age = calculateAge(profile.birthday);
    if (age !== null) {
        contacts.push(`<span class="resume-contact">
                                    ${renderIcon(iconSpriteUrl, 'calendar')}
    ...
    if (phone) {
        contacts.push(`<span class="resume-contact" aria-label="${escapeHtml(formatMessage(text.phoneAria, { phone }))}">
                                    ${renderIcon(iconSpriteUrl, 'phone')}
    ...
    if (profile.email) {
        ...
        contacts.push(`<span class="resume-email-group">
                                    ...
                                    <button class="icon-button resume-copy-button" type="button" data-copy-email="${escapeHtml(email)}" aria-label="${escapeHtml(text.copyEmail)}" aria-describedby="resumeActionStatus" title="${escapeHtml(text.copyEmail)}">
                                        ${renderIcon(iconSpriteUrl, 'copy', 'resume-copy-icon')}
                                        ${renderIcon(iconSpriteUrl, 'check', 'resume-copy-success-icon')}
                                    </button>
                                </span>`);
    }
...
function renderAnnouncementsSection(announcements, text, iconSpriteUrl) {
    ...
    const items = activeAnnouncements.map((item, index) => {
        const link = item.link && item.link.label && item.link.url
            ? `<a href="${safeUrl(item.link.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.link.label)}${renderIcon(iconSpriteUrl, 'external-link')}</a>`
            : '';
        return `<div class="announcement-slide" data-announcement-slide data-expires-at="${escapeHtml(item.expiresAt || '')}" aria-hidden="${index === 0 ? 'false' : 'true'}">
    ...
    const indicators = activeAnnouncements.map((item, index) => {
        return `<button type="button" class="announcement-dot" data-announcement-index="${index}" aria-label="${escapeHtml(formatMessage(text.announcementIndex, { index: index + 1 }))}" aria-pressed="${index === 0 ? 'true' : 'false'}"></button>`;
    }).join('');
...
function renderResumeSection(headingId, title, icon, content, iconSpriteUrl) {
    if (!content) return '';
    return `<section class="resume-section" aria-labelledby="${headingId}">
                    <h2 class="resume-section-title" id="${headingId}">${renderIcon(iconSpriteUrl, icon, 'resume-section-icon')}${escapeHtml(title)}</h2>
                    ${content}
                </section>`;
}

function renderResumeEntries(items, iconSpriteUrl) {
    if (!items.length) return '';
    return `<div class="resume-entry-list">${items.map(item => {
        const description = item.descriptionUrl
            ? `<a class="resume-entry-link" href="${safeUrl(item.descriptionUrl)}" target="_blank" rel="noopener noreferrer"><span>${escapeHtml(item.description)}</span>${renderIcon(iconSpriteUrl, 'external-link')}</a>`
...
function renderPortfolioSection(items, text, profile, headingId, icon, linkLabel, iconSpriteUrl) {
    if (!items.length) return '';
    ...
    const entries = items.map(item => {
        ...
        const link = url ? `<div class="work-links"><a href="${url}" target="_blank" rel="noopener noreferrer">${escapeHtml(linkLabel)}${renderIcon(iconSpriteUrl, 'external-link')}</a></div>` : '';
    ...
    return renderResumeSection(headingId, text[headingId], icon, `<div class="works-list">${entries}</div>`, iconSpriteUrl);
}
...
function layoutValues(config, locale, iconSpriteUrl, activePage) {
    ...
    const nav = mobile => PAGE_IDS.filter(id => config.pages[id]).map(id => {
        ...
        const icon = mobile ? renderIcon(iconSpriteUrl, icons[id]) : '';
...
function renderBookmarks(folders, text, iconSpriteUrl) {
    ...
    const bookmarkTagFilter = bookmarkTagCounts.size
        ? `<div class="bookmark-tag-filter" id="bookmarkTagFilter" aria-label="${escapeHtml(text.BOOKMARK_TAG_FILTER_ARIA)}">
                        <button type="button" class="bookmark-tag-filter-button" data-bookmark-tag="" aria-pressed="true">${escapeHtml(text.BOOKMARK_TAG_ALL)} <span>${bookmarkTotal}</span></button>
    ...
    const bookmarks = folders.map((folder, idx) => {
        const links = folder.links.map(l => {
            ...
            return `<${tag}${href} class="bookmark-link" data-bookmark-url="${escapeHtml(l.url)}">
                            <span class="bookmark-link-heading"><span>${escapeHtml(l.label)}</span>${l.url ? renderIcon(iconSpriteUrl, 'external-link') : ''}</span>
        ...
        return `<div class="bookmark-category" data-bookmark-category="${escapeHtml(folder.name)}">
                    <button type="button" class="category-header" aria-expanded="${idx === 0 ? 'true' : 'false'}" aria-controls="${groupId}">
                        ...
                        ${renderIcon(iconSpriteUrl, 'chevron-down', 'category-toggle')}
                    </button>
...
function pageValues(config, seo, locale, iconSpriteUrl, activePage, runtimeKeys, route = '') {
    const text = UI_TEXT[locale];
    const runtimeText = Object.fromEntries(['themeToLight', 'themeToDark', ...runtimeKeys].map(key => [key, text[key]]));
    return {
        ...layoutValues(config, locale, iconSpriteUrl, activePage),
        HTML_LANG: locale === 'en' ? 'en' : 'zh-CN',
        SITE_NAME: escapeHtml(config.profile.siteName),
        SITE_ICON: escapeHtml(config.profile.siteIcon),
        SITE_FAVICON: createTextFavicon(config.profile.siteIcon),
        PROFILE_NAME: escapeHtml(config.profile.name),
        ICON_SPRITE_URL: escapeHtml(iconSpriteUrl),
        LANG_SWITCH_URL: (locale === 'en' ? '/' : '/en/') + route,
        LANG_SWITCH_HREFLANG: locale === 'en' ? 'zh-CN' : 'en',
        LANG_SWITCH_LABEL: locale === 'en' ? '中' : 'EN',
        RUNTIME_CONFIG: `window.PAGE_I18N = ${JSON.stringify(runtimeText).replace(/</g, '\\u003c')};\n`,
        FOOTER: renderFooter(config.profile, seo, text),
        VISITOR_COUNTER_SCRIPT: renderVisitorCounterScript(config.profile)
    };
}

function writePage(route, locale, html) {
    const outputPath = path.join(ROOT_DIR, 'dist', locale === 'en' ? 'en' : '', route, 'index.html');
...
function buildHomepage(config, seo, locale, assetManifest) {
    ...
    const values = pageValues(config, seo, locale, iconSpriteUrl, enabledPageIds[0], MAIN_RUNTIME_TEXT_KEYS);
    values.ENABLED_PAGE_IDS = JSON.stringify(enabledPageIds);
    values.RUNTIME_CONFIG += `window.ENABLED_PAGE_IDS = ${values.ENABLED_PAGE_IDS};\n`;
    values.STYLESHEET_URL = resolveBuiltAssetUrl(assetManifest.stylesheet, locale);
    values.MAIN_SCRIPT_URL = resolveBuiltAssetUrl(assetManifest.mainScript, locale);
    ...
    values.PDF_DOWNLOAD = fs.existsSync(path.join(ROOT_DIR, pdfFile))
        ? `<a class="button button-secondary button-small resume-pdf-download" href="/${pdfFile}" download="${escapeHtml(createPdfFilename(config.profile, locale))}">${renderIcon(iconSpriteUrl, 'download')}${escapeHtml(text.downloadPdf)}</a>`
        : '';
    ...
    values.HERO_LINKS = renderProfileLinks(config.profile.links, iconSpriteUrl);
    values.ANNOUNCEMENTS_SECTION = renderAnnouncementsSection(config.announcements, text, iconSpriteUrl);
    values.RESUME_CONTACTS = renderResumeContacts(config.profile, text, iconSpriteUrl);
    values.RESEARCH_INTERESTS = renderResearchInterests(config.profile, text);
    values.EDUCATION_SECTION = renderResumeSection('educationHeading', text.educationHeading, 'scholar', renderResumeEntries(config.education, iconSpriteUrl), iconSpriteUrl);
    values.STUDENT_WORK_SECTION = renderResumeSection('studentWorkHeading', text.studentWorkHeading, 'users', renderResumeEntries(config.studentWork, iconSpriteUrl), iconSpriteUrl);
    values.AWARDS_SECTION = renderResumeSection('awardsHeading', text.awardsHeading, 'award', renderResumeEntries(config.awards, iconSpriteUrl), iconSpriteUrl);
    values.PAPERS_SECTION = renderPortfolioSection(config.papers, text, config.profile, 'publicationsHeading', 'book-open', text.paperLink, iconSpriteUrl);
    values.PROJECTS_SECTION = renderPortfolioSection(config.projects, text, config.profile, 'projectsHeading', 'grid', text.projectLink, iconSpriteUrl);
    values.APPS_DIRECTORY = renderAppDirectory(text, iconSpriteUrl);

    Object.assign(values, renderBookmarks(config.bookmarks, text, iconSpriteUrl));
    const html = renderTemplate('modern.template.html', values, text, config.pages);

    writePage('', locale, html);
...
function buildAppPages(config, seo, locale, assetManifest) {
    ...
    const assetUrl = asset => resolveBuiltAssetUrl(asset, locale, 2);
    const iconSpriteUrl = assetUrl(assetManifest.iconSprite);
    ...
    return APPS.map(app => {
        ...
        const values = {
            ...pageValues(config, seo, locale, iconSpriteUrl, 'apps', app.runtime, `apps/${app.id}/`),
            APP_CONTENT: content,
            APP_LAYOUT_CLASS: app.contentWidth === 'wide' ? '' : ' app-detail-container-game',
            APP_TITLE: escapeHtml(text[app.title]), APP_DESCRIPTION: escapeHtml(text[app.description]),
            APP_URL: appUrl, ZH_APP_URL: zhAppUrl, EN_APP_URL: enAppUrl, APPS_URL: `${localeRoot}#apps`,
            SITE_STYLESHEET_URL: assetUrl(assetManifest.appBaseStylesheet), APP_STYLESHEET_URL: assetUrl(assetManifest.apps[app.id].stylesheet),
            APP_SCRIPT_URL: assetUrl(assetManifest.apps[app.id].script)
        };
        const html = renderTemplate('app.template.html', values, text, config.pages);

        writePage(`apps/${app.id}`, locale, html);
